...
public class FileModifyChecker
{
	...
	public void start() throws Exception
	{
		...
		String envPath = SystemSetting.getInstance().getString(EnvKey.FileName.FRAMEWORK, EnvKey.ENV_PATH);
					...
					if(wk != null)
					{
						...
						for(WatchEvent<?> event : events)
						{
							...
							if(changed.toString().endsWith("service.xml"))
							{
								try
								{
									String envHome = SystemSetting.getInstance().getFramework(EnvKey.ENV_PATH);
