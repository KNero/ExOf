...
//		return dword;
	...
	public static byte[] subBytes(byte[]src, int startPos, int length) {
		if( src.length<length ) {
			return null;
		}
		else if( startPos + length>src.length ) {
			return null;
		}
		
		byte[] b = new byte[length];
		
		System.arraycopy(src, startPos, b, 0, length);
	...
	public static byte[] concat(byte[] src1, byte[] src2)
	{
		...
		System.arraycopy(src1, 0, b, 0, src1.length);
		System.arraycopy(src2, 0, b, src1.length, src2.length);
		
		return b;
	}
	
	public static void intToByte(int value, byte[] data, int idx) {
